...
class SampleNativeCustomEventLoader private constructor(
    ...
    private val TAG = "APP@@@"
    private val TAG1 = "SampleNativeCustomEventLoaderprivate"
    companion object{

        @SuppressLint("StaticFieldLeak")
        @Volatile
        private var INSTANCE: SampleNativeCustomEventLoader? = null

        fun getInstance(maxAdapterResponseParameters: MaxAdapterResponseParameters?,
                        activity: Activity?,
                        callback: MaxNativeAdAdapterListener?): SampleNativeCustomEventLoader {
            return INSTANCE ?: synchronized(this) {
                INSTANCE ?: SampleNativeCustomEventLoader(maxAdapterResponseParameters,activity,callback).also {
                    INSTANCE = it
                }
            }
        }
    ...
    fun loadAd(){
        ...
        TapMindsMediationAdapter.getInstance(activity!!).loadNativeAd(request,activity!!, object : TapMindNativeAdAdapterListener{
            ...
            override fun onNativeAdLoaded(tapMindNativeAd: TapMindNativeAd?, bundle: Bundle?) {
                ...
                tapMindNativeAd?.let {
                    val maxNativeAd = SampleUnifiedNativeAdMapper(tapMindNativeAd)
