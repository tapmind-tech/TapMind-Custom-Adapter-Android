...
//        }
    ...
    private fun fetchAdFromServer(
        ...
        val request = AdRequest(
            ...
            adPayload?.adType,
            adPayload?.country,
            adPayload?.adapterName,
//            adPayload?.continent,
//            adPayload?.region,
            adPayload?.sdkVersion
        )
            ...
            withContext(Dispatchers.Main) {
                when (response) {
                    is Success -> {
                        ...
                        val adapters = data.adapters

                        if (data == null || adapters.isNullOrEmpty()) {
                            onFailure(
                                TapMindAdapterError(204, "No adapters in response")
                            )
                            return@withContext
                        }

                        val sortedAdapters = adapters.sortedBy {
                            it.priority ?: Int.MAX_VALUE
                        }

                        Log.e("TapMindAdapterAdmob", "fetchAdFromServer: ${response.data}")
                        onSuccess(sortedAdapters, data, adPayload?.adapterName ?: "")
                    }
