...
//    }
    ...
    override fun loadAdViewAd(
        ...
        fetchAdFromServer(onSuccess = { adapters, adData ->
            if (adapters[0].partner == "GAM" || adapters[0].partner == "AdMob") {
                AdMobManager.getInstance().loadAdViewAd(
                    adData, adapters, params, format, activity as Activity, listener,
                )
        ...
        fetchAdFromServer(onSuccess = { adapters, adData ->
            if (adapters[0].partner == "GAM" || adapters[0].partner == "AdMob") {
                AdMobManager.getInstance().loadInterstitialAd(
                    adData, parameters, adapters, context, callback
                )
            } else if (adapters[0].partner == "IronSource") {
            ...
            onSuccess = { adapters, adData ->
                if (adapters[0].partner == "GAM" || adapters[0].partner == "AdMob") {
                    ...
                    IronSourceManager.getInstance()
                        .loadRewardedAd(adData, parameters, adapters, context, callback)
                }
            },
            onFailure = { error ->
...
//        FbManager.getInstance().loadNativeAd(parameters!!,  activity as Activity, callback!!)
        fetchAdFromServer(onSuccess = { adapters, adData ->
            AdMobManager.getInstance().loadNativeAd(
                adData, var1!!, adapters, var2!!, var3!!
            )
        }, onFailure = { error ->
    ...
    private fun fetchAdFromServer(
        onSuccess: (List<DataItem>, AdData) -> Unit,
            ...
            withContext(Dispatchers.Main) {
                when (response) {
                    is Success -> {
                        ...
                        onSuccess(sortedAdapters, response.data)
                    }
